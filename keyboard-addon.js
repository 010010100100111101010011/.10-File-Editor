(function () {
    ...
    if (!isMobile) return; // Exit if desktop
    ...
    function renderKeyboard() {
        ...
        if (currentMode === 'ICHING') {
            ...
            enterBtn.className = 'kbd-btn kbd-wide kbd-enter';
        ...
        currentLayout.forEach((row, rowIndex) => {
            ...
            row.forEach(key => {
                ...
                if (key === 'SHIFT') {
                    ...
                    displayKey = currentMode === 'SYM' ? 'ABC' : '?';
                    btn.classList.add('kbd-wide');
                } else if (key === 'ALPHA') {
                    displayKey = 'ABC';
                    btn.classList.add('kbd-wide');
                    ...
                    displayKey = '↵';
                    btn.classList.add('kbd-wide', 'kbd-enter');
                }
    ...
    function handleKeyPress(key) {
        ...
        if (key === 'SHIFT') {
            ...
            textInput.value = currentValue.substring(0, start) + '\n' + currentValue.substring(end);
            textInput.setSelectionRange(start + 1, start + 1);
        } else {
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        if (textInput && textInput.parentNode) {
            // Disable default Android / iOS software keyboard
